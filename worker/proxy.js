...
export default {
  async fetch(request, env) {
    ...
    if (!targetUrl) {
      return corsResponse(new Response('Missing url parameter', { status: 400 }));
    }
    ...
    if (!['http:', 'https:'].includes(parsedUrl.protocol)) {
      return corsResponse(new Response('Only HTTP(S) allowed', { status: 400 }));
    }

    // SSRF protection
    if (BLOCKED_PATTERN.test(parsedUrl.hostname)) {
      return corsResponse(new Response('Private addresses not allowed', { status: 403 }));
    }
    ...
    try {
      ...
      return corsResponse(new Response('Proxy error', { status: 502 }));
    }
  },
};
