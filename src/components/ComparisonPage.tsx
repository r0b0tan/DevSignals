...
// =============================================================================
// DESIGN TOKENS - Indigo-600 Only
// =============================================================================

const tokens = {
  // Label Level 1 - Neutral/Informational
  label1: {
    text: 'text-indigo-600',
    border: 'border-indigo-600',
    bg: 'bg-white',
  },
  // Label Level 2 - Emphasized/Comparison-Relevant
  label2: {
    text: 'text-indigo-600',
    border: 'border-indigo-600',
    bg: 'bg-indigo-50',
  },
  // Label Level 3 - Primary/Signal
  label3: {
    text: 'text-white',
...
function Label({ level, children }: LabelProps) {
  const style = tokens[`label${level}`];
  return (
    <span
      className={`inline-flex items-center rounded px-2 py-0.5 text-xs font-medium border ${style.text} ${style.border} ${style.bg}`}
    >
      {children}
    </span>
  );
...
function RatioIndicator({ value }: { value: number }) {
  // value is 0-1, higher = more divs/spans (worse semantic markup)
  ...
  const levelNum: LabelLevel = level === 'low' ? 1 : level === 'medium' ? 2 : 3;

  return (
    <div className="flex items-center gap-2">
      <Label level={levelNum}>{labelMap[level]}</Label>
      <span className="text-xs tabular-nums text-gray-400">{percent.toFixed(0)}%</span>
    </div>
  );
}

function H1Badge({ count }: { count: number }) {
  // H1 count: 0 = bad, 1 = ideal, >1 = warning
  if (count === 0) {
    return <Label level={3}>Missing</Label>;
  }
  if (count === 1) {
    return <Label level={1}>1</Label>;
  }
  return <Label level={2}>{count} (multiple)</Label>;
}

function SignalLabel({ struct, sem }: { struct: string; sem: string }) {
  // Map classifications to signal labels
...
function IssueCount({ count }: { count: number }) {
  ...
  return <Label level={count > 5 ? 3 : 2}>{count}</Label>;
}
...
const CORE_METRICS: MetricDefinition[] = [
  // Structure (3 metrics)
  {
    ...
    render: (v) => <span className="text-sm tabular-nums">{Number(v).toLocaleString()}</span>,
  },
  {
    key: 'maxDepth',
    label: 'Max DOM Depth',
  ...
  // Semantics (3 metrics)
  {
    ...
    render: (v) => <H1Badge count={Number(v)} />,
  },
  {
  ...
  },
  {
    ...
    render: (v) => <RatioIndicator value={Number(v)} />,
  },
  // Quality (1 metric)
  {
    key: 'linkIssues',
    ...
    render: (v) => <IssueCount count={Number(v)} />,
  },
];
...
function MetricGroup({ title, children }: MetricGroupProps) {
  return (
    <div className="border-b border-gray-100 last:border-b-0">
      <div className="bg-gray-50/60 px-4 py-1.5">
...
function MetricRow({ label, values, isIdentical, showIdentical }: MetricRowProps) {
  ...
  return (
    <div
      className={`grid items-center border-b border-gray-50 py-2 ${
        isIdentical ? 'opacity-50' : ''
      }`}
      style={{ gridTemplateColumns: `160px repeat(${values.length}, 1fr)` }}
    >
      <div className="sticky left-0 bg-white px-4 text-sm text-gray-500">{label}</div>
      {values.map((value, i) => (
...
function UrlHeader({ entries, onRemove }: UrlHeaderProps) {
  return (
    <div
      className="sticky top-0 z-10 grid border-b border-gray-200 bg-white"
      style={{ gridTemplateColumns: `160px repeat(${entries.length}, 1fr)` }}
    >
      <div className="sticky left-0 bg-white px-4 py-3" />
      {entries.map((entry, i) => (
        <div key={i} className="flex items-start justify-between px-4 py-3">
          ...
          {onRemove && entries.length > 2 && (
            <button
              onClick={() => onRemove(i)}
              className="ml-2 rounded p-0.5 text-gray-300 hover:bg-gray-100 hover:text-gray-500"
            >
...
function SignalRow({ entries }: SignalRowProps) {
  return (
    <div
      className="grid items-center border-b border-gray-200 bg-gray-50/30 py-3"
      style={{ gridTemplateColumns: `160px repeat(${entries.length}, 1fr)` }}
    >
      <div className="sticky left-0 bg-gray-50/30 px-4 text-sm font-medium text-gray-700">
        Result
      </div>
      {entries.map((entry, i) => (
        <div key={i} className="px-4">
